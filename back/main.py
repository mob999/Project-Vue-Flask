...
opt = ''
def detect(img_path, save_img=False):
    classify, out, source, view_img, save_txt, imgsz = \
       opt.classify, opt.output, img_path, opt.view_img, opt.save_txt, opt.img_size
    webcam = source == '0' or source.startswith('rtsp') or source.startswith('http') or source.endswith('.txt')
    with open('./config.json', 'r') as f:
        config = json.load(f)
        det_weights = config["det_weights"]
        rec_weights = config["rec_weights"]
    # if classify:
    #     print('classify')
    # Initialize
...
def run(img_path):
    ...
    parser.add_argument('--rec-weights', nargs='+', type=str, default='./weights/Final_LPRNet_model.pth', help='model.pt path(s)')
